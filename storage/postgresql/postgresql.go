...
type Store struct {
	...
	promo_codes storage.PromoRepoI
}
...
func NewConnectPostgresql(cfg *config.Config) (storage.StorageI, error) {
	...
	return &Store{
		...
		promo_codes: NewPromoRepo(pgpool),
	}, nil
}
...
func (s *Store) PromoCode() storage.PromoRepoI{
	if s.promo_codes == nil {
		s.promo_codes = NewPromoRepo(s.db)
	}
	return s.promo_codes
}
