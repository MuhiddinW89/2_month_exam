...
func (r *staffRepo) GetStaffSellsById(ctx context.Context, req *models.StaffPrimaryKey) (*models.GetStaffSellsById, error) {
	var (
		query string
		staff models.GetStaffSellsById
	)
	...
	query = `
		...
		FROM staffs AS stf
		JOIN orders AS o ON stf.staff_id = o.staff_id
	...
	staff.OrderItemData = &models.OrderItem{}
	staff.OrderData = &models.Order{}

	err := r.db.QueryRow(ctx, query, req.StaffId).Scan(
		&staff.FirstName,
		&staff.CategoryData.CategoryName,
		&staff.OrderItemData.Quantity,
		&staff.OrderData.OrderDate,
	)
	if err != nil {
		return nil, err
	}
