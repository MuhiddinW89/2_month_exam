...
// TransferStock godoc
// @ID transfer_stock
// @Router /stock/transfer [PUT]
// @Summary Transfer Stock
// @Description Transfer Stock
// @Tags Stock
// @Accept json
// @Produce json
// @Param transfer body models.TransferStock true "Transfer Request Body"
// @Success 200 {object} Response{data=string} "Success Request"
// @Response 400 {object} Response{data=string} "Bad Request"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) TransferStock(c *gin.Context) {
	var req models.TransferStock
	if err := c.ShouldBindJSON(&req); err != nil {
	h.handlerResponse(c, "transfer stock", http.StatusBadRequest, "invalid request body")
	return
	}
	
	if err := h.storages.Stock().Transfer(context.Background(), &req); err != nil {
		h.handlerResponse(c, "storage.stock.transfer", http.StatusInternalServerError, err.Error())
		return
	}
	
	h.handlerResponse(c, "transfer stock response", http.StatusOK, "Stock transfer successful")
	}
