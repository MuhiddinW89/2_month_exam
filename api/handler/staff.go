...
// Get By ID Staff Sells godoc
// @ID get_by_sell__id_staff
// @Router /staff/sells/{id} [GET]
// @Summary Get By ID Staff Sells
// @Description Get By ID Staff Sells
// @Tags Staff
// @Accept json
// @Produce json
// @Param id path string true "id"
...
// @Response 400 {object} Response{data=string} "Bad Request"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) GetByIdStaffSell(c *gin.Context) {

	id := c.Param("id")

	idInt, err := strconv.Atoi(id)
	if err != nil {
		h.handlerResponse(c, "storage.staffSell.getByID", http.StatusBadRequest, "id incorrect")
		return
	}

	resp, err := h.storages.Staff().GetStaffSellsById(context.Background(), &models.StaffPrimaryKey{StaffId: idInt})
	if err != nil {
		h.handlerResponse(c, "storage.staffSell.getByID", http.StatusInternalServerError, err.Error())
		return
	}
