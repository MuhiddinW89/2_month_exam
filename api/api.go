...
func NewApi(r *gin.Engine, cfg *config.Config, store storage.StorageI, logger logger.LoggerI) {
	...
	r.PUT("/stock/transfer", handler.TransferStock)
	...
	r.GET("/staff/sells/:id", handler.GetByIdStaffSell)
	...
	r.POST("/order_item/totalSum", handler.OrdeerTotalSum)

	// prmo_codes api
	r.POST("/promo_codes", handler.CreatePromoCode)
	r.GET("/promo_codes/:id", handler.GetByIdPromoCode)
	r.GET("/promo_codes", handler.GetListPromoCode)
	r.DELETE("/promo_codes/:id", handler.DeletePromoCode)
